...
namespace ngh5
{
  namespace graph
  {
                              ...
                              )
    {
      ...
      if (delim != -3)
        {
          printf("rank %d: unpack_rank_edge_map: recvcounts[%u] = %d rdispls[%u] = %d recvpos = %d recvbuf_size = %u delim = %d\n", 
                 rank, ridx, recvcounts[ridx], ridx, rdispls[ridx], recvpos, recvbuf_size, delim);
        }
      assert(delim == -3);
#endif
