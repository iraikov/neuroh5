...
namespace ngh5
{
  namespace model
  {
    ...
    template<>
    const map< NODE_IDX_T, vector<float> >& AttrMap::attr_map<float> (size_t i) const
    {
      return float_values[i];
    }
    template<>
    const map< NODE_IDX_T, vector<uint8_t> >& AttrMap::attr_map<uint8_t> (size_t i) const
    {
      return uint8_values[i];
    }
    template<>
    const map< NODE_IDX_T, vector<int8_t> >& AttrMap::attr_map<int8_t> (size_t i) const
    {
      return int8_values[i];
    }
    template<>
    const map< NODE_IDX_T, vector<uint16_t> >& AttrMap::attr_map<uint16_t> (size_t i) const
    {
      return uint16_values[i];
    }
    template<>
    const map< NODE_IDX_T, vector<uint32_t> >& AttrMap::attr_map<uint32_t> (size_t i) const
    {
      return uint32_values[i];
    }
    template<>
    const map< NODE_IDX_T, vector<int32_t> >& AttrMap::attr_map<int32_t> (size_t i) const
    {
      return int32_values[i];
    }

    template<>
    const vector<map< NODE_IDX_T, vector<float> > >& AttrMap::attr_maps<float> () const
    {
      return float_values;
    }
    template<>
    const vector<map< NODE_IDX_T, vector<uint8_t> > >& AttrMap::attr_maps<uint8_t> () const
    {
      return uint8_values;
    }
    template<>
    const vector<map< NODE_IDX_T, vector<int8_t> > >& AttrMap::attr_maps<int8_t> () const
    {
      return int8_values;
    }
    template<>
    const vector<map< NODE_IDX_T, vector<uint16_t> > >& AttrMap::attr_maps<uint16_t> () const
    {
      return uint16_values;
    }
    template<>
    const vector<map< NODE_IDX_T, vector<uint32_t> > >& AttrMap::attr_maps<uint32_t> () const
    {
      return uint32_values;
    }
    template<>
    const vector<map< NODE_IDX_T, vector<int32_t> > >& AttrMap::attr_maps<int32_t> () const
    {
      return int32_values;
    }
    ...
    template<>
    const vector<vector<float>> AttrMap::find<float> (NODE_IDX_T idx)
    {
    ...
    template<>
    const vector<vector<uint8_t>> AttrMap::find<uint8_t> (NODE_IDX_T idx)
    {
    ...
    template<>
    const vector<vector<int8_t>> AttrMap::find<int8_t> (NODE_IDX_T idx)
    {
    ...
    template<>
    const vector<vector<uint16_t>> AttrMap::find<uint16_t> (NODE_IDX_T idx)
    {
    ...
    template<>
    const vector<vector<uint32_t>> AttrMap::find<uint32_t> (NODE_IDX_T idx)
    {
    ...
    template<>
    const vector<vector<int32_t>> AttrMap::find<int32_t> (NODE_IDX_T idx)
    {
    ...
    template<>
    size_t AttrMap::insert (const std::vector<NODE_IDX_T> &idx,
      ...
      for (size_t p=0; p<idx.size(); p++)
        {
          ...
          float_values[index].insert(make_pair(vidx, v));
          idx_set.insert(vidx);
        }
      return index;
    }
      ...
      for (size_t p=0; p<idx.size(); p++)
        {
          ...
          uint8_values[index].insert(make_pair(vidx, v));
          idx_set.insert(vidx);
        }
      return index;
    }
      ...
      for (size_t p=0; p<idx.size(); p++)
        {
          ...
          int8_values[index].insert(make_pair(vidx, v));
          idx_set.insert(vidx);
        }
      return index;
    }
      ...
      for (size_t p=0; p<idx.size(); p++)
        {
          ...
          uint16_values[index].insert(make_pair(vidx, v));
          idx_set.insert(vidx);
        }
      return index;
    }
      ...
      for (size_t p=0; p<idx.size(); p++)
        {
          ...
          uint32_values[index].insert(make_pair(vidx, v));
          idx_set.insert(vidx);
        }
      return index;
    }
      ...
      for (size_t p=0; p<idx.size(); p++)
        {
          ...
          int32_values[index].insert(make_pair(vidx, v));
          idx_set.insert(vidx);
        }
      return index;
    }
    ...
    size_t NamedAttrMap::insert (std::string name,
                                 ...
                                 const std::vector<float> &value)
    {
      size_t index = AttrMap::insert(idx, ptr, value);
