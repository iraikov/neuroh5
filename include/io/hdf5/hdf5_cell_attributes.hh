...
namespace neurotrees
{
  ...
  template <typename T>
  herr_t hdf5_read_cell_attribute
  (
   MPI_Comm                  comm,
   const hid_t&              loc,
   const std::string&        path,
    ...
    if (read_size > 0)
      {
        ...
        rank_ranges(read_size, size, ranges);
        ...
        if (block > 0)
          {
            ...
            status = hdf5_read<NODE_IDX_T> (loc, gid_path,
                                            start, block,
            ...
            ptr.resize(block+1);
            status = hdf5_read<ATTR_PTR_T> (loc, ptr_path,
                                            start, block+1,
            ...
            status = hdf5_read<T> (loc, value_path,
  ...
  template <typename T>
  void hdf5_append_cell_attribute
  (
   const hid_t&                    loc,
   const std::string&              path,
   ...
   )
  {
    ...
    status = hdf5_write<CELL_IDX_T> (file, path + "/gid",
                                     global_gid_size, local_gid_start, local_gid_size,
                                     CELL_IDX_H5_NATIVE_T,
                                     gid, wapl);
    
    status = hdf5_write<ATTR_PTR_T> (file, path + "/ptr",
                                     global_ptr_size, local_ptr_start, local_ptr_size,
                                     ATTR_PTR_H5_NATIVE_T,
                                     local_attr_ptr, wapl);
    
    status = hdf5_write<T> (file, path + "/value",
  ...
  template <typename T>
  void hdf5_write_cell_attribute
  (
   MPI_Comm                        comm,
   const hid_t&                    loc,
   const std::string&              path,
    ...
    if (global_value_size > 0)
      {
        ...
        status = hdf5_write<CELL_IDX_T> (loc, path + "/gid",
                                         global_gid_size, local_gid_start, local_gid_size,
                                         CELL_IDX_H5_NATIVE_T,
                                         gid, wapl);
        
        status = hdf5_write<ATTR_PTR_T> (loc, path + "/ptr",
                                         global_ptr_size, local_ptr_start, local_ptr_size,
                                         ATTR_PTR_H5_NATIVE_T,
                                         local_attr_ptr, wapl);
        
        status = hdf5_write<T> (loc, path + "/value",
                                global_value_size, local_value_start, local_value_size,
                                mtype, value, wapl);
        status = H5Pclose(wapl);
        assert(status == 0);
