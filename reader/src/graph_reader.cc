...
namespace ngh5
{
  ...
  /*****************************************************************************
   * Append entries to a map where the key is a dst node and the value
   * is all src nodes that connect to it.
   *****************************************************************************/
  int append_dst_edge_map
  (
   const NODE_IDX_T&         dst_start,
   const NODE_IDX_T&         src_start,
   const vector<DST_BLK_PTR_T>&  dst_blk_ptr,
  ...
  /*****************************************************************************
   * Append entries to a map where the key is a src node and the value
   * is all dst nodes that it connects to.
   *****************************************************************************/
  int append_src_edge_map
  (
   const NODE_IDX_T&         dst_start,
   const NODE_IDX_T&         src_start,
   const vector<DST_BLK_PTR_T>&  dst_blk_ptr,
                ...
                if (i < dst_ptr_size-1) 
                  {
                    ...
                    for (size_t j = low; j < high; ++j)
                      {
                        NODE_IDX_T src = src_idx[j] + src_start;
                        rank_t srcrank = node_rank_vector[src];

                        pair<rank_edge_map_iter_t,bool> r = rank_edge_map.insert(make_pair(srcrank, edge_map_t()));
                        pair<edge_map_iter_t,bool> n = rank_edge_map[srcrank].insert(make_pair(src, edge_tuple_t()));

                        edge_tuple_t& et = rank_edge_map[srcrank][src];
   ...
   )
  {
    ...
    uint32_t numitems = src_vector.size();
    assert(MPI_Pack(&numitems, 1, MPI_UINT32_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
    ...
    for (size_t k = 0; k < edge_attr_values.size_attr_vec<uint8_t>(); k++)
      {
        uint32_t numitems = edge_attr_values.size_attr<uint8_t>(k);
        assert(MPI_Pack(&numitems, 1, MPI_UINT32_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
    ...
    for (size_t k = 0; k < edge_attr_values.size_attr_vec<uint16_t>(); k++)
      {
        uint32_t numitems = edge_attr_values.size_attr<uint16_t>(k);
        assert(MPI_Pack(&numitems, 1, MPI_UINT32_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
    ...
    for (size_t k = 0; k < edge_attr_values.size_attr_vec<uint32_t>(); k++)
      {
        uint32_t numitems = edge_attr_values.size_attr<uint32_t>(k);
        assert(MPI_Pack(&numitems, 1, MPI_UINT32_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
        assert(MPI_Pack(edge_attr_values.attr_ptr<uint8_t>(k), numitems,
                        MPI_UNSIGNED_CHAR, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
      }
  ...
  /*****************************************************************************
   * Prepare an MPI packed data structure with destination vertices
   * and edge attributes for a given source vertex.
   *****************************************************************************/

  int pack_src_edge
  (
   MPI_Comm comm,
   const NODE_IDX_T &src,
   const vector<NODE_IDX_T>& dst_vector,
   const EdgeAttr& edge_attr_values,
   int &sendpos,
   int &sendsize,
   vector<uint8_t> &sendbuf
   )
  {
    ...
    assert(MPI_Pack_size(1, MPI_UINT32_T, comm, &packsize) == MPI_SUCCESS);
    sendsize += packsize;
    assert(MPI_Pack_size(dst_vector.size(), NODE_IDX_MPI_T, comm, &packsize) == MPI_SUCCESS);
    sendsize += packsize;
    ...
    // Create MPI_PACKED object with all the destination vertices and edge attributes
    assert(MPI_Pack(&src, 1, NODE_IDX_MPI_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
  
    uint32_t numitems = dst_vector.size();
    assert(MPI_Pack(&numitems, 1, MPI_UINT32_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
    assert(MPI_Pack(&dst_vector[0], dst_vector.size(), NODE_IDX_MPI_T,
                    &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);

    for (size_t k = 0; k < edge_attr_values.size_attr_vec<float>(); k++)
      {
        uint32_t numitems = edge_attr_values.size_attr<float>(k);
        assert(MPI_Pack(&numitems, 1, MPI_UINT32_T, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
        assert(MPI_Pack(edge_attr_values.attr_ptr<float>(k), numitems,
                        MPI_FLOAT, &sendbuf[0], sendbuf_size, &sendpos, comm) == MPI_SUCCESS);
      }
    ...
    for (size_t k = 0; k < edge_attr_num[0]; k++)
      {
        ...
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &numitems, 1, MPI_UINT32_T, comm);
        vec.resize(numitems);
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &vec[0], numitems, MPI_FLOAT, comm);
    ...
    for (size_t k = 0; k < edge_attr_num[1]; k++)
      {
        ...
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &numitems, 1, MPI_UINT32_T, comm);
        vec.resize(numitems);
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &(vec)[0], numitems, MPI_UNSIGNED_CHAR, comm);
        edge_attr_values.insert(vec);
      }
    ...
    for (size_t k = 0; k < edge_attr_num[2]; k++)
      {
        ...
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &numitems, 1, MPI_UINT32_T, comm);
        vec.resize(numitems);
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &(vec)[0], numitems, MPI_UNSIGNED_SHORT, comm);
        edge_attr_values.insert(vec);
      }
    ...
    for (size_t k = 0; k < edge_attr_num[3]; k++)
      {
        ...
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &numitems, 1, MPI_UINT32_T, comm);
        vec.resize(numitems);
        MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &(vec)[0], numitems, MPI_UNSIGNED, comm);
        edge_attr_values.insert(vec);
      }
  ...
  /*****************************************************************************
   * Unpack an MPI packed edge data structure into destination vertices and edge attributes
   *****************************************************************************/
   ...
   )
  {
    ...
    assert(MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &src, 1, NODE_IDX_MPI_T, comm) >= 0);

    MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &numitems, 1, MPI_UINT32_T, comm);

    dst_vector.resize(numitems);
    MPI_Unpack(&recvbuf[0], recvbuf_size, &recvpos, &dst_vector[0], numitems, NODE_IDX_MPI_T, comm);
        ...
        if (rank < io_size)
          {
            ...
            assert(append_dst_edge_map(dst_start, src_start, dst_blk_ptr, dst_idx, dst_ptr, src_idx,
                        ...
                        if (src_vector.size() > 0)
                          {
                            ...
                            assert(pack_dst_edge(all_comm, dst, src_vector, my_edge_attrs,
            ...
            while ((unsigned int)recvpos < recvbuf.size()-1)
              {
                ...
                unpack_dst_edge(all_comm, dst, src_vector, edge_attr_values, recvpos, recvbuf, edge_attr_num);
        ...
        if (rank < io_size)
          {
            ...
            // append to the edge map
            assert(append_src_edge_map(dst_start, src_start, dst_blk_ptr, dst_idx, dst_ptr, src_idx,
                                       edge_attr_values, node_rank_vector, local_prj_num_edges, prj_rank_edge_map) >= 0);
            ...
            for (auto it1 = prj_rank_edge_map.cbegin(); it1 != prj_rank_edge_map.cend(); ++it1)
              {
                uint32_t src_rank;
                src_rank = it1->first;
                sdispls[src_rank] = sendpos;
                if (it1->second.size() > 0)
                  {
                    for (auto it2 = it1->second.cbegin(); it2 != it1->second.cend(); ++it2)
                      {
                        int sendsize;
                        NODE_IDX_T src = it2->first;
                  
                        const vector<NODE_IDX_T>  dst_vector = get<0>(it2->second);
                        const EdgeAttr&      my_edge_attrs = get<1>(it2->second);

                        num_packed_edges = num_packed_edges + dst_vector.size();
                        
                        if (dst_vector.size() > 0)
                          {
                      
                            assert(pack_src_edge(all_comm, src, dst_vector, my_edge_attrs,
                                                 sendpos, sendsize, sendbuf) == 0);
                      
                            sendcounts[src_rank] += sendsize;
                          }
                      }
                  }
            ...
            while ((unsigned int)recvpos < recvbuf.size()-1)
              {
                ...
                unpack_src_edge(all_comm, src, dst_vector, edge_attr_values, recvpos, recvbuf, edge_attr_num);
                num_recv_edges = num_recv_edges + dst_vector.size();
                
                if (prj_edge_map.find(src) == prj_edge_map.end())
                  {
                    prj_edge_map.insert(make_pair(src,make_tuple(dst_vector, edge_attr_values)));
                  }
                else
                  {
                    edge_tuple_t et = prj_edge_map[src];
                    vector<NODE_IDX_T> &v = get<0>(et);
                    EdgeAttr &a = get<1>(et);
                    v.insert(v.end(),dst_vector.begin(),dst_vector.end());
                    a.append(edge_attr_values);
                    prj_edge_map[src] = make_tuple(v,a);
                  }
              }
          }
        recvbuf.clear();
